...
# {{whoToCheck}(nick)}'s Incan Farm Empire
...
`☀️ Normal Weather Total Yield`
> + 🥔 {m:trunc({potatofarms.current}*30)}

`🌊 El Nino Flood Total Yield`
> + 🥔 {m:trunc({potatofarms.current}*35)}

## Llama Farms ({llamafarms.current})
# {llamaFarmPrint({noLEmojiToPrint})}
`☀️ Normal Weather Total Yield`
> + 🥔 {m:trunc({llamafarms.current}*50)}

`🌊 El Nino Flood Total Yield`
> + 🥔 {m:trunc({llamafarms.current}*0)}

## Silos ({silos.current})
# {siloPrint({noSEmojiToPrint})}
`🛡️ Locust Protection`
> + 🥔 {m:trunc({silos.current}*50)}

### Locust Attacks
||Unfortunately these are not automatically tracked and are also single use. Please check your subs to review how many you've purchased this round!||