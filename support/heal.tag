{=(COMMENT):Basically editdmg.tag but removes all damage counter}
{silence}

{=(hasShield):1201853062411730995}{=(baseHP):1201782450565419038}{=(hasBaseHP):{if({contains({baseHP}):{user(roleids)}}==true): true|false}}{=(hasHylianShield):{if({contains({hasShield}):{user(roleids)}}==true): true|false}}{=(maxHP):{m:trunc({if({hasBaseHP}==true):5|0}{if({hasHylianShield}==true):+3|+0})}}
